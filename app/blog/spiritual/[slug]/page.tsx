...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    return {
      title: `${title} - Spiritual Blog`,
      description,
      alternates: {
        canonical: `${baseUrl}/blog/spiritual/${post.slug}`,
      },
      openGraph: {
        title,
        description,
        type: 'article',
        publishedTime: date,
        url: `${baseUrl}/blog/spiritual/${post.slug}`,
        images: [
          {
            url: ogImage,
          },
...
export default async function SpiritualBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    // Verify this is actually a spiritual post - if not, show 404
    if (post.frontMatter.category !== 'spiritual') {
      notFound()
    }
    ...
    const blogPostingSchema = {
      ...
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `${baseUrl}/blog/spiritual/${post.slug}`,
      },
    ...
    console.error('Error in SpiritualBlogPostPage:', error)
    notFound()
  }
} 