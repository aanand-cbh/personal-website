...
export async function generateStaticParams() {
  try {
    ...
    // Get all posts and filter for tech category only
    ...
    // Only return slugs for tech posts
    const techPosts = allPosts
      .filter((post): post is NonNullable<typeof post> => post !== null)
      .filter(post => post.frontMatter.category === 'tech')
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    return {
      ...
      description,
      alternates: {
        canonical: `${baseUrl}/blog/tech/${post.slug}`,
      },
      openGraph: {
        title,
        description,
        type: 'article',
        publishedTime: date,
        url: `${baseUrl}/blog/tech/${post.slug}`,
        images: [
          {
            url: ogImage,
          },
...
export default async function TechBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    // Verify this is actually a tech post - if not, show 404
    ...
    const blogPostingSchema = {
      ...
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `${baseUrl}/blog/tech/${post.slug}`,
      },
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                <Badge variant="outline">Tech & Software Engineering</Badge>
